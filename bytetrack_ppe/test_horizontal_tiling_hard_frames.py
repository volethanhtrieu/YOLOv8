...
OUTPUT_DIR = ROOT / "outputs" / "horizontal_tiling_test"
...
def make_tiles(frame):
    ...
    # 1x2 horizontal slicing with overlap.
    # The source video is 16:9, so splitting only across width
    # already makes each tile close to square. That raises the
    # effective person resolution almost as much as 2x2 slicing,
    # while requiring only two inference passes instead of four.
    tile_w = int(
        math.ceil(
            width / (TILE_COLS - (TILE_COLS - 1) * TILE_OVERLAP)
        )
    )
    ...
    if TILE_COLS == 1:
        ...
        max_start = max(0, width - tile_w)
        x_starts = [
            int(round(i * max_start / (TILE_COLS - 1)))
...
def make_comparison(
    ...
    tiled_view = add_header(
        ...
        [
            (
                f"Frame {frame_index} | "
                f"1x2 HORIZONTAL TILED overlap={TILE_OVERLAP:.0%}"
            ),
            (
